...
В этом задании мы разработаем аналог unix pipeline, что-то вроде:

```bash
cat common.go | grep func | sort | uniq -c | sort -n  
```

когда STDOUT одной программы передаeтся как STDIN в другую программу, но в нашем случае эти роли выполняют каналы, которые мы передаeм из одной функции в другую.

Cамо задание по сути состоит из двух частей:
...
то есть мы получаем на вход email пользователей, селектим их из "базы" и получаем каждому юзеру `user_id`. Затем селектим по каждому `user_id` список писем(`msg_id`) этого юзера. Дальше проверяем эти письма на спам и выдаем итоговый результат: какие у письма со спамом, а какие нет.

Теперь подробнее про реализацию этой цепочки:
...
* `SelectMessages()`
  ...
  * особенности:
    ...
    * `GetMessages()` позволяет использовать **"батчи"**. то есть за раз в нее можно запихнуть не 1 юзера, а несколько. максимальное кол-во юзеров = `2`.  То есть если мы хотим селектнуть у 10ти юзеров письма, то это можно сделать за 5 вызовов `GetMessages()`. в тестах проверяется, что кол-во вызовов оптимальное
* `CheckSpam()`
...
TL;DR
- Реализацию (код) писать в `spammer.go`.
- Для установки необходимых зависимостей `go get github.com/stretchr/testify/assert`.
- Запустить программу `go run .`.
...
* задание построено так чтобы хорошо разобраться со всем материалом, т.е. вдумчиво посмотреть примеры и применить их на практике. Искать по гуглу или стек оферфлоу ничего не надо!
* вам не надо накапливать данные - сразу передаeм их дальше (Разве что функция сама не решает накопить - у нас это CombineResults)
...
Эталонное решение занимает 185 строк
